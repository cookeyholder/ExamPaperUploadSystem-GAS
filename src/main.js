...
async function initApp() {
    try {
        ...
        document.getElementById("user-display-name").textContent =
            `${user.name} (${user.role === "admin" ? "管理員" : "教師"})`;

        const users = await MockApiService.getTableData("users");
        const settings = await MockApiService.getTableData("settings");

        console.log(
            `[Mock API] System initialized with ${users.length} users and ${settings.length} active exam schedules.`,
        );
    } catch (error) {

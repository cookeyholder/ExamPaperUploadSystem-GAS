...
export class SubjectMgmt {
  static async render() {
    ...
    // Task 1: Table Layout
    const rowsHtml = examRows.map(row => {
      const assignedTeacher = teachers.find(t => t.email === row.teacherEmail);
      const teacherName = assignedTeacher ? assignedTeacher.name : '<span class="text-danger"><i class="bi bi-exclamation-circle me-1"></i>尚未指派</span>';
    ...
    const teacherOptions = teachers.map(t => `<option value="${t.email}">${t.name} (${t.email})</option>`).join('');

  ...
  static async render() {
    ...
    const rowsHtml = settings.map(row => {
      ...
      if (!row.uploadStart || !row.uploadEnd) {
        statusHtml = '<span class="badge bg-secondary bg-opacity-10 text-secondary"><i class="bi bi-dash-circle me-1"></i>未設定</span>';
      } else if (now < start) {
        statusHtml = '<span class="badge bg-warning bg-opacity-10 text-warning"><i class="bi bi-clock me-1"></i>尚未開始</span>';
      } else if (now > end) {
        statusHtml = '<span class="badge bg-danger bg-opacity-10 text-danger"><i class="bi bi-x-circle me-1"></i>已截止</span>';
      } else {
        statusHtml = '<span class="badge bg-success bg-opacity-10 text-success"><i class="bi bi-check-circle me-1"></i>開放中</span>';
      }
    ...
    document.querySelectorAll('.edit-plan-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (rowData) {
          ...
          document.getElementById('ep-name').textContent = `${rowData.academicYear}-${rowData.semester} ${rowData.examName}`;

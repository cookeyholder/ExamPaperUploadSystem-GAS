...
export class GasApiService {
  ...
  static async addTableRow(tableName, dataObj) {
      return new Promise((resolve, reject) => {
          if (typeof google !== 'undefined' && google.script) {
              const mappedDataObj = GasApiService.mapDataToBackend(tableName, dataObj);
              google.script.run
                  .withSuccessHandler(res => {
  ...
  static async updateTableRow(tableName, keyField, keyValue, updateObj) {
      return new Promise((resolve, reject) => {
          if (typeof google !== 'undefined' && google.script) {
              const mappedUpdateObj = GasApiService.mapDataToBackend(tableName, updateObj);
              const mappedKeyField = GasApiService.mapKeyToBackend(tableName, keyField);
              google.script.run
                  .withSuccessHandler(res => {
                      const result = JSON.parse(res);
                      if (result.error) reject(new Error(result.error));
                  ...
                  .withFailureHandler(err => reject(err))
                  .apiUpdateTableRow(tableName, mappedKeyField, keyValue, mappedUpdateObj);
          } else {
              reject(new Error("無 GAS 環境"));
          }
      });
  ...
  static mapDataToBackend(tableName, frontendData) {
      ...
      if (tableName === 'users') {
          ...
          if (frontendData.teacherCode !== undefined) map['人員編號'] = frontendData.teacherCode;
          if (frontendData.department !== undefined) map['部門單位'] = frontendData.department;
          ...
          if (frontendData.academicYear !== undefined) map['學年度'] = frontendData.academicYear;
          if (frontendData.semester !== undefined) map['學期'] = frontendData.semester;
          if (frontendData.examName !== undefined) map['考試名稱'] = frontendData.examName;
          if (frontendData.uploadStart !== undefined) map['開始時間'] = frontendData.uploadStart;
          if (frontendData.uploadEnd !== undefined) map['結束時間'] = frontendData.uploadEnd;
      } else if (tableName.startsWith('exam')) {
          if (frontendData.id !== undefined) map['ID'] = frontendData.id;
          if (frontendData.department !== undefined) map['類別'] = frontendData.department;
          if (frontendData.grade !== undefined) map['年級'] = frontendData.grade;
          if (frontendData.subject !== undefined) map['科目名稱'] = frontendData.subject;
          if (frontendData.teacherEmail !== undefined) map['命題教師'] = frontendData.teacherEmail;
          if (frontendData.markingType !== undefined) map['閱卷方式'] = frontendData.markingType;
          if (frontendData.pageCount !== undefined) map['試卷張數'] = frontendData.pageCount;
          if (frontendData.fileUrl !== undefined) map['檔案連結'] = frontendData.fileUrl;
      }
